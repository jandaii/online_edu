...
<body data-type="chart">
 ...
 String uri="jdbc:mysql://rm-2vcwo56w973ev4a09pm.mysql.cn-chengdu.rds.aliyuncs.com:3306/work";
        task_name= request.getParameter("taskname");
        byte bb[]=task_name.getBytes("iso-8859-1");
            task_name=new String(bb);
 try{
...
<input type ='hidden' name='youxiu' value="<%= T_id%>">                       
  ...
  int task_id=0;
   ...
   try {  Class.forName("com.mysql.jdbc.Driver");
     ...
     out.print(" <th class='table-author am-hide-sm-only'>详情</th>");
...
<div class="am-cf">
                ...
                <div class="portlet-title">
                    <div class="caption font-green bold">
                        <span class="am-icon-code"></span> 正确率
                    </div>
                    <div class="tpl-portlet-input tpl-fz-ml">
                        <div class="portlet-input input-small input-inline">
  ...
  // ResultSet rs;
   try {  Class.forName("com.mysql.jdbc.Driver");
      ...
      try{ 
           con=DriverManager.getConnection(uri,"work","swufework");
     sql=con.createStatement();
    String cdn="SELECT * FROM subject where (Subject_type='"+type1+"'or "
           + "Subject_type='"+type2+"' or Subject_type='"+type3+"') AND  Task_name='"+task_name+"'";
      ResultSet rs_an=sql.executeQuery(cdn);
     
   
     out.print(" <table class='am-table am-table-striped am-table-hover table-main'>");
     out.print("<thead>");
     out.print("<tr>");
     out.print("<th class='table-author am-hide-sm-only'>题目名称</th>");
     out.print("<th class='table-title'>题目内容</th>");
     out.print("<th class='table-type'>题目类型</th>");
     out.print(" <th class='table-author am-hide-sm-only'>正确答案</th>");
     out.print(" <th class='table-author am-hide-sm-only'>正确率</th>");
    ...
    // out.print("<th><font color=blue>放入购物车</font>");
     ...
     String detailMess="";
     while(rs_an.next()){
       String sb_na=rs_an.getString(3);
       String sb_cont =rs_an.getString(4);
      String ty=rs_an.getString(5);
    Float sb_sc=rs_an.getFloat(6);
     String answer=rs_an.getString(7);
      int sbid=rs_an.getInt(1);
       String cdn2="SELECT COUNT(Student_No) FROM answer where Subject_id='"+sbid+"'and student_answer='"+answer+"' GROUP BY Subject_id";
       Statement sql2; 
       sql2 = con.createStatement();
       ResultSet rs_ancount=sql2.executeQuery(cdn2);
       Float count_right=1f;
     while(rs_ancount.next()){
        count_right=rs_ancount.getFloat(1);
     }
         String cdn3="SELECT COUNT(Student_No) FROM answer where Subject_id='"+sbid+"' GROUP BY Subject_id";
         Statement sql3; 
       sql3 = con.createStatement();
     ResultSet rs_all=sql3.executeQuery(cdn3);   
     Float all_stu=1f;
     while(rs_all.next()){
         all_stu=rs_all.getFloat(1);
     }
      /* double c_r=count_right*1.0; 
  double a_s=all_stu*1.0; 
  double fen=c_r/a_s; 

    //##.00%   百分比格式，后面不足2位的用0补齐 
   //baifenbi=nf.format(fen);   */

//String rate = df1.format(count_right/all_stu);  
//double Rate=Double.parseDouble(rate);
       

        out.print("<tr>");
        out.print("<td>"+sb_na+"</td>");
        out.print("<td>"+sb_cont+"</td>");
        out.print("<td>"+ty+"</td>");
        out.print("<td>"+sb_sc+"</td>");
        out.print("<td>");
        out.print("<div class='am-btn-toolbar'>");
        out.print("<div class='am-btn-group am-btn-group-xsr'>");
         out.print(decimalFormat.format(count_right*100/all_stu)+"%");
        out.print("</div>");
         out.print("</div>");
        out.print("</td>");
        out.print("</tr>");
