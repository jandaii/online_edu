...
<html class="no-js">
    <head>
        <jsp:useBean id="dataBean4" class="mybean.data.Bean_xinxibypage4" scope="request"/>
    ...
    <body>
        ...
        <div class="about_con">
            <div class="wrap">
                <div class="left">
                    <ul class="s_list">
                        <li><a href="Servlet_gonggao?a=1">通知公告</a></li>
                        <li><a href="Servlet_gonggao2?a=1">学校新闻</a></li>
                        <li><a href="Servlet_gonggao3?a=1">学术讲座</a></li>
                        <li><a href="Servlet_gonggao4?a=1" class="curr">前沿动态</a></li>
                    </ul>
                </div>
                <div class="right">
			<h1 class="a_tit">【前沿动态】</h1>
                        <p style="line-height:2.6em;font-size:16px;">
                             <table align=center><tr><td>
                        <jsp:getProperty name="dataBean4"  property="backNews" /> 
                         <jsp:setProperty name="dataBean4" property="pageSize" param="pageSize"/>
<jsp:setProperty name="dataBean4" property="currentPage" param="currentPage"/>
<%    
      CachedRowSetImpl rowSet=dataBean4.getRowSet();
      if(rowSet==null) {
      ...
      int pageSize=dataBean4.getPageSize();  //每页显示的记录数
      int totalPages = dataBean4.getTotalPages();
      if(totalRecord%pageSize==0)
           totalPages = totalRecord/pageSize;//总页数
      else
           totalPages = totalRecord/pageSize+1;
      dataBean4.setPageSize(pageSize);
      dataBean4.setTotalPages(totalPages);
      if(totalPages>=1) {
         if(dataBean4.getCurrentPage()<1)
              dataBean4.setCurrentPage(dataBean4.getTotalPages());
         if(dataBean4.getCurrentPage()>dataBean4.getTotalPages())
              dataBean4.setCurrentPage(1);
         int index=(dataBean4.getCurrentPage()-1)*pageSize+1;
         ...
         for(int i=1;i<=pageSize&&boo;i++) {   
            ...
            session.setAttribute("content",content); 
              String detail="<form  action='Guanli_gonggao_details.jsp' method = 'post'>"+
                     ...
                     "<input type ='hidden' name='type' value='前沿动态'>"+
...
%></td></tr>
                             <tr><td>
 <BR>每页最多显示<jsp:getProperty name="dataBean4" property="pageSize"/>条信息
 <BR>当前显示第<Font color=blue>
     <jsp:getProperty name="dataBean4" property="currentPage"/>
   </Font>页,共有
   <Font color=blue><jsp:getProperty name="dataBean4" property="totalPages"/>
   </Font>页。
                                 </td></tr>
  <tr><td><FORM action="" method=post>
          <Input type=hidden name="currentPage" value=
         "<%=dataBean4.getCurrentPage()-1 %>">
           <Input type=submit name="g" value="上一页"></FORM></td>
      <td><FORM action="" method=post>
          <Input type=hidden name="currentPage"
           value="<%=dataBean4.getCurrentPage()+1 %>">
